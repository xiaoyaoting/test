...
class WalletAuthenticator extends BaseAuthenticator {
   ...
   * @returns {Promise<object>} { appPk, authInfo }
   */
   ...
   * @param {string} [params.context.mfaCode] - function used to generate mfa code
  ...
  async sign({ context, request, claims, pathname = '', baseUrl = '', challenge = '', extraParams = {} }) {
    ...
    if (context.currentStep === 0 && context.sharedKey && get(request, 'query._ek_')) {
      try {
        const pk = fromBase58(get(request, 'query._ek_')).toString('utf8');
