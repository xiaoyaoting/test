...
const { encrypt, decrypt } = require('../protocol');
...
const isConnectedOnly = (params, sessionUserDid = '') => {
  ...
  // query string from client: `true` | `false`
...
const PROTECTED_KEYS = ['challenge', 'sharedKey'];
...
module.exports = function createHandlers({
  ...
  const onProcessError = ({ req, res, stage, err }) => {
    ...
    if (token) {
      tokenStorage.update(token, { status: STATUS_ERROR, error: err.message, mfaCode: 0 });
    }
  ...
  const generateSession = async (req, res) => {
    try {
      ...
      const sharedKey = getStepChallenge(); // used for wallet to encrypt userInfo
      ...
      await tokenStorage.update(token, { currentStep: 0, challenge, sharedKey, extraParams: params, appInfo });
  ...
  const onAuthRequest = async (req, res) => {
    ...
    if (params.version && store.clientVersion !== params.version) {
      store.clientVersion = params.version;
      await tokenStorage.update(token, { clientVersion: params.version });
    }
    ...
    try {
      ...
      if (shouldSkipConnect) {
        ...
        if (extraParams.forceConnected) {
          set(steps, '[0].authPrincipal.target', extraParams.connectedDid);
        }
      }
      ...
      const signedClaim = await authenticator.sign(
        Object.assign(signParams, {
          context: {
            ...
            userPk,
            didwallet,
            ...pick(store, ['currentStep', 'sharedKey']),
            mfaCode: createMfaCodeGenerator(token),
          },
    ...
    try {
      ...
      const onLastStep = async (result) => {
        ...
        if (nextWorkflow && result && result.nextWorkflowData) {
          ...
          if (previousWorkflowData.length > 10240) {
            const err = new Error('base64 encoded nextWorkflowData should be less than 10240 characters');
