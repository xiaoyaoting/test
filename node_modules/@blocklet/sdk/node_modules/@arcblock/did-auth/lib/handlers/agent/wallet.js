...
class AgentWalletHandlers extends WalletHandlers {
   ...
   */
  constructor({ tokenStorage, agentStorage, authenticator, onConnect = noop, options = {} }) {
    if (options && !options.prefix) {
      options.prefix = '/api/agent/:authorizeId';
    }

    super({ tokenStorage, authenticator, onConnect, options });
  ...
  attach({
    ...
    app.get(`${prefix}/${action}/token`, ensureAuthorization, generateSession);
    app.post(`${prefix}/${action}/token`, ensureAuthorization, generateSession);

    // 2. WEB: check for token status
    app.get(`${prefix}/${action}/status`, ensureAuthorization, ensureContext, checkSession);

    // 3. WEB: to expire old token
    app.get(`${prefix}/${action}/timeout`, ensureAuthorization, ensureContext, expireSession);

    // 4. Wallet: fetch auth request
    app.get(pathname, ensureAuthorization, ensureContext, ensureSignedJson, onAuthRequest);

    // 5. Wallet: submit auth response
    app.post(pathname, ensureAuthorization, ensureContext, ensureSignedJson, onAuthResponse);

    // 6. Wallet: submit auth response for web wallet
    app.get(`${pathname}/submit`, ensureAuthorization, ensureContext, ensureSignedJson, onAuthResponse);
  }
}
