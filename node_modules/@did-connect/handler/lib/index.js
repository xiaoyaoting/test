...
function createHandlers({ storage, authenticator, logger = console, socketPathname = '/api/connect/relay/websocket', }) {
    ...
    const handleSessionUpdate = (context) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            return yield storage.update(context.sessionId, updates);
        }
        catch (err) {
            logger.error(err);
    ...
    const handleSessionDelete = (context) => __awaiter(this, void 0, void 0, function* () {
        const { session, sessionId } = context;
        const result = verifyUpdater(context, session.updaterPk);
        if (result.error) {
            return result;
        }
        yield storage.delete(sessionId);
        return { code: 'OK' };
    });
    ...
    const waitForSession = (sessionId, timeout, checkFn, reason, locale) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            return yield storage.read(sessionId);
        }
        catch (err) {
    ...
    const fetchRequestList = (session, locale) => __awaiter(this, void 0, void 0, function* () {
        try {
            const result = yield axios_1.default.post(session.connectUrl, Object.assign(Object.assign({}, (0, pick_1.default)(session, ['sessionId', 'authUrl', 'currentConnected', 'previousConnected'])), { locale }), { timeout: session.timeout.app });
    ...
    const fetchApproveResult = (session, locale) => __awaiter(this, void 0, void 0, function* () {
        try {
            const result = yield axios_1.default.post(session.approveUrl, Object.assign(Object.assign({}, (0, pick_1.default)(session, [
                'sessionId',
                'authUrl',
                'challenge',
                'currentStep',
                ...
                'responseClaims',
                'approveResults',
            ])), { locale }), { timeout: session.timeout.app });
            return result.data;
        }
        catch (err) {
            if (err.response) {
        ...
        try {
            ...
            const handleWalletApprove = () => __awaiter(this, void 0, void 0, function* () {
                ...
                const justifiedClaims = ((0, isEmpty_1.default)(claims) ? [{ type: 'authPrincipal' }] : claims).map((x) => {
                    ...
                    if ((0, isEmpty_1.default)(x)) {
                        return Object.assign({ type: 'authPrincipal' }, (session.currentConnected || { userDid, userPk }));
                    }
                    if (x.type === 'authPrincipal') {
                        return Object.assign(Object.assign({}, x), (session.currentConnected || { userDid, userPk }));
                    }
                    return x;
                });
                logger.info('session.walletApproved', sessionId, justifiedClaims);
                const updated = yield storage.update(sessionId, {
                ...
                if (session.approveUrl) {
                    const approveResults = [...session.approveResults, yield fetchApproveResult(updated, locale)];
                ...
                yield waitForAppApprove(sessionId, session.timeout.app, locale);
            ...
            if (session.status === 'walletScanned') {
                ...
                if (session.onlyConnect) {
                    newSession = yield handleWalletApprove();
